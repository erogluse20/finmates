function LinkFeedItem(post) {
    ...
    const followBtnHTML = isSelf ? `
        <button class="follow-btn" onclick="triggerEditPortfolio(${post.id})" style="cursor: pointer; position: relative; z-index: 10; border-color: var(--text-tertiary); color: var(--text-secondary);">
             Düzenle <i class="ph-fill ph-pencil-simple"></i>
        </button>
    ` : `
        ...
        <article class="portfolio-card">
            ...
            <div class="card-content">
                ...
                <div class="card-stats">
                    ...
                    ${(post.stats && post.stats.different > 0) ? `<div class="stat-item stat-count-different" style="color:var(--text-secondary)"><i class="ph-fill ph-brain" style="color:#06b6d4"></i> <span class="count-val">${post.stats.different}</span></div>` : ''}
            ...
            return post.assets.map((asset, index) => {
                const dashLength = (asset.percent / 100) * circumference;
                    ...
                    <div class="donut-legend">
                        ${post.assets.map(asset => `
                            <div class="legend-item">
                                <div class="legend-color" style="background-color: ${asset.color || 'var(--color-brand)'}"></div>
                                <span class="legend-label">${asset.symbol}</span>
                                <span class="legend-value">${asset.percent}%</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
...
window.toggleReaction = async (id, type) => {
    ...
    if (container) {
        ...
        const config = {
            ...
            'different': { icon: 'ph-fill ph-brain', class: 'btn-reacted-different', text: 'Farklı', color: '#06b6d4' },
