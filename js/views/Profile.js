function ProfileView() {
    ...
    const getMyPortfolios = () => {
        // We merged MOCK and Local in app.js for feed, let's do same here or just check local
        // If the user is creating new portfolios, they are in 'finmates_posts'
        // If we want to show MOCK posts that belong to this user (Demo User), we should include them
        const local = JSON.parse(localStorage.getItem('finmates_posts') || '[]');
        const mock = window.MOCK_POSTS || [];
        const all = [...local, ...mock];
        return all.filter(p => !p.user || p.user.username === user.username || p.user.username_handle === user.username);
    };
    ...
    const getLikedPortfolios = () => {
        ...
        const mock = window.MOCK_POSTS || [];
        const all = [...local, ...mock];
        return all.filter(p => likes.includes(p.id));
    };

    const myPortfolios = getMyPortfolios();
        ...
        <div class="profile-content">
            <div id="my-portfolios-content">
                ...
                <div id="my-portfolio-list">
                    ${myPortfolios.length > 0 ? myPortfolios.map(p => renderPortfolioItem(p, true)).join('') : `
